...
wss.on('connection', function connection(ws, req) {
    ...
    ws.on('close', function close() {
        ...
        if (clientType === 'broadcaster') {
            handleBroadcasterDisconnect(ws);
...
function handleBroadcasterJoin(ws, message) {
    ...
    // 如果broadcasterId包含用戶ID，保持完整的broadcaster_前綴
    if (broadcasterId && broadcasterId.startsWith('broadcaster_')) {
        // 保持完整的broadcaster_用戶名格式，避免重複
        broadcasterId = broadcasterId;
    } else if (!broadcasterId) {
        broadcasterId = `broadcaster_${Date.now()}`;
    } else {
        // 如果沒有broadcaster_前綴，添加前綴
        broadcasterId = `broadcaster_${broadcasterId}`;
    }
    
    // 不再使用全局 broadcaster 變數，只使用 activeBroadcasters
    console.log('🔍 [DEBUG] 添加主播到 activeBroadcasters:', broadcasterId);
    console.log('🔍 [DEBUG] 添加前的 activeBroadcasters 大小:', activeBroadcasters.size);
    ...
    console.log('🔍 [DEBUG] 添加後的 activeBroadcasters 大小:', activeBroadcasters.size);
    console.log('🔍 [DEBUG] 當前所有主播ID:', Array.from(activeBroadcasters.keys()));
...
function handleChatJoin(ws, message) {
    let broadcasterId = message.broadcasterId || 'default';
    
    // 確保 broadcasterId 格式與 activeBroadcasters 的鍵值格式一致
    if (broadcasterId !== 'default' && !broadcasterId.startsWith('broadcaster_')) {
        broadcasterId = `broadcaster_${broadcasterId}`;
    }
    
    console.log('用戶加入聊天:', message.role, message.username, '直播間:', broadcasterId);
    ...
    console.log(`用戶已添加到聊天列表: ${message.username} (${message.role}) 直播間: ${broadcasterId}`);
...
function handleChatMessage(ws, message) {
    ...
    console.log('收到聊天訊息:', messageText, '來自:', username, '角色:', role, '直播間:', broadcasterId);
    ...
    try {
        ...
        // 只廣播給同一個直播間的ChatSystem用戶
        broadcastToChatUsersInRoom(chatMessage, broadcasterId);
        
        console.log('已廣播聊天訊息到直播間:', broadcasterId, '訊息:', chatMessage.message, '來自:', chatMessage.username, '角色:', chatMessage.role);
...
// 專門廣播給特定直播間的ChatSystem用戶
function broadcastToChatUsersInRoom(message, broadcasterId) {
    const messageStr = JSON.stringify(message);
    let broadcastCount = 0;
    
    console.log(`🔍 [DEBUG] 開始廣播到直播間 ${broadcasterId}`);
    console.log(`🔍 [DEBUG] 當前聊天用戶列表:`, Array.from(chatUsers.entries()).map(([ws, info]) => ({
        username: info.username,
        role: info.role,
        broadcasterId: info.broadcasterId
    })));
    
    // 只發送給同一個直播間的聊天用戶（ChatSystem連接）
    chatUsers.forEach((userInfo, ws) => {
        console.log(`🔍 [DEBUG] 檢查用戶 ${userInfo.username} (${userInfo.role}), 直播間: ${userInfo.broadcasterId}, 目標: ${broadcasterId}`);
        
        if (ws.readyState === WebSocket.OPEN && userInfo.broadcasterId === broadcasterId) {
            try {
                ws.send(messageStr);
                broadcastCount++;
                console.log(`✅ [聊天廣播] 發送給直播間 ${broadcasterId}: ${userInfo.username} (${userInfo.role})`);
            } catch (error) {
            ...
            console.log(`❌ [聊天廣播] 跳過用戶 ${userInfo.username}: WebSocket狀態=${ws.readyState}, 直播間不匹配=${userInfo.broadcasterId !== broadcasterId}`);
        }
    });
    
    console.log(`[聊天廣播] 已發送給直播間 ${broadcasterId} 的 ${broadcastCount} 個用戶`);
}
...
async function handleUnifiedChat(payload, senderWs, clientType, clientId){
    try {
        ...
        const chatPacket = {
            ...
            timestamp: ts,
            tempId,
            broadcasterId: payload.broadcasterId || 'default'
        };
        ...
        let delivered = 0;
        let broadcasterId = payload.broadcasterId || 'default';
        ...
        console.log('🔍 [DEBUG] 統一聊天廣播，broadcasterId:', broadcasterId);
        
        // 廣播給同一個直播間的觀眾
        viewers.forEach((vws, vid) => { 
            if(vws.readyState === WebSocket.OPEN) { 
                // 檢查觀眾是否在同一個直播間
                const viewerConnection = Array.from(activeConnections.values())
                    .find(conn => conn.type === 'viewer' && conn.ws === vws);
                
                if (viewerConnection && viewerConnection.streamerId === broadcasterId) {
                    if(sendJSON(vws, chatPacket)) delivered++; 
                    else console.log('[WARN] 發送觀眾失敗 viewerId=', vid); 
                }
            } 
        });
        ...
        if(broadcasterData && broadcasterData.ws && broadcasterData.ws.readyState === WebSocket.OPEN) { 
            sendJSON(broadcasterData.ws, chatPacket); 
            delivered++;
        }
...
// 處理主播斷線
function handleBroadcasterDisconnect(ws) {
    console.log('主播斷線');
    
    // 找到對應的主播ID
    let broadcasterId = null;
    for (const [id, data] of activeBroadcasters.entries()) {
        if (data.ws === ws) {
            broadcasterId = id;
            break;
        }
...
app.get('/api/live-streams', async (req, res) => {
    try {
        ...
        // 遍歷所有活躍的直播會話
        console.log('🔍 [DEBUG] activeBroadcasters 總數:', activeBroadcasters.size);
        console.log('🔍 [DEBUG] activeBroadcasters 鍵值:', Array.from(activeBroadcasters.keys()));
        
        for (const [userId, streamerData] of activeBroadcasters.entries()) {
            console.log('🔍 [DEBUG] 檢查主播:', userId, 'WebSocket狀態:', streamerData.ws ? streamerData.ws.readyState : 'null');
