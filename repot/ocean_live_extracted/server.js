...
const ALLOWED_PAYMENT_METHODS = Object.keys(PAYMENT_METHOD_LABELS);

const GIFT_CATALOG = {
    heart: { label: '心心', amount: 5 },
    flower: { label: '花朵', amount: 5 },
    rocket: { label: '火箭', amount: 1000 },
    cake: { label: '蛋糕', amount: 100 },
    gun: { label: '槍', amount: 150 },
    glasses: { label: '眼鏡', amount: 10 },
    poop: { label: '便便', amount: 5 },
    diamond: { label: '鑽石', amount: 500},
    car: { label: '跑車', amount: 2000},    
    star: { label: '星星', amount: 10},
    crown: { label: '皇冠', amount: 5000},
    kiss: { label: '飛吻', amount: 10},
    hug: { label: '擁抱', amount: 20},
    rose: { label: '玫瑰', amount: 5},
    icecream: { label: '冰淇淋', amount: 12},
    balloon: { label: '氣球', amount: 8},
    fireworks: { label: '煙火', amount: 200},
    magicwand: { label: '魔法棒', amount: 75},
    trophy: { label: '獎盃', amount: 10000},
    money: { label: '鈔票', amount: 20},
    headset: { label: '耳機', amount: 15}
};
...
function mapGiftRow(row) {
    ...
    const gift = GIFT_CATALOG[row.gift_type] || {};

    return {
        id: row.id,
        giftType: row.gift_type,
        giftLabel: gift.label || row.gift_type,
        amount: row.amount,
...
async function sendPasswordResetEmail(recipientEmail, resetLink) {
    ...
    console.log('📧 [Password Reset] 預備向使用者發送重設郵件:', recipientEmail);
    console.log('🔗 重設密碼連結:', resetLink);
    return true;
}
...
function parseIntegerIdCandidate(value) {
...
async function resolveBroadcasterUserId({ broadcasterId = null, activeEntry = null } = {}) {
    const userInfo = activeEntry && activeEntry.userInfo ? activeEntry.userInfo : {};
    ...
    for (const candidate of numericCandidates) {
        const parsed = parseIntegerIdCandidate(candidate);
        if (parsed !== null) {
            return parsed;
        }
    }
    ...
    const stringCandidates = [
        ...
        userInfo.userName,
        userInfo.email,
        typeof broadcasterId === 'string' ? broadcasterId : null
    ].filter(Boolean);

    for (const identifier of stringCandidates) {
        const cleaned = typeof identifier === 'string' ? identifier.trim() : '';
        if (!cleaned) {
            continue;
        }

        try {
            const lookup = cleaned.includes('@')
                ? await db.getUserByEmail(cleaned)
                : await db.getUserByUsername(cleaned);

            if (lookup && lookup.id) {
                return lookup.id;
            }
        } catch (lookupErr) {
            if (!lookupErr || lookupErr.message !== '用戶不存在') {
                console.warn('resolveBroadcasterUserId lookup failed:', cleaned, lookupErr && lookupErr.message);
            }
        }
    }
...
function selectFallbackBroadcasterEntry() {
    // 優先選擇正在直播的主播
    for (const entry of activeBroadcasters.entries()) {
        const data = entry[1];
...
function handleBroadcasterJoin(wss, message) {
    ...
    // 檢查是否有等待中的觀眾
    console.log(`🔍 檢查是否有等待主播 ${broadcasterId} 的觀眾...`);
    let waitingViewersCount = 0;
    let waitingViewersList = [];
    
    for (const [viewerId, viewerData] of allViewers) {
        if (viewerData.streamerId === broadcasterId) {
            // 找到等待中的觀眾
            console.log(`👉 發現等待中的觀眾: ${viewerId}`);
            
            // 將觀眾添加到主播的觀眾列表
            const viewerWs = viewerData.ws;
            if (viewerWs && viewerWs.readyState === WebSocket.OPEN) {
                activeBroadcasters.get(broadcasterId).viewers.set(viewerId, viewerWs);
                activeBroadcasters.get(broadcasterId).viewerCount++;
                waitingViewersCount++;
                waitingViewersList.push(viewerId);
    ...
    if (waitingViewersCount > 0) {
        console.log(`✅ 已將 ${waitingViewersCount} 名等待中的觀眾加入主播列表`);
        
        // 通知主播有觀眾等待連接
        wss.send(JSON.stringify({
            type: 'viewers_need_connection',
            count: waitingViewersCount,
            viewers: waitingViewersList,
            message: `有 ${waitingViewersCount} 名觀眾正在等待連接`
        }));
    }
...
function handleViewerJoin(wss, message) {
    const viewerIdRaw = message.viewerId || `viewer_${Date.now()}`;
    const viewerId = viewerIdRaw.toString();
    const requestedStreamerId = message.streamerId !== undefined && message.streamerId !== null
        ? message.streamerId.toString().trim()
        : '';
    let streamerId = requestedStreamerId || 'default';
    const normalizedKey = streamerId ? streamerId.toLowerCase() : 'default';
    const shouldAutoAssign = !streamerId || normalizedKey === 'default' || normalizedKey === 'auto' || normalizedKey === 'latest';
    let targetBroadcaster = shouldAutoAssign ? null : activeBroadcasters.get(streamerId);
    let autoAssigned = false;

    if (!targetBroadcaster && shouldAutoAssign) {
        const fallbackEntry = selectFallbackBroadcasterEntry();
        if (fallbackEntry) {
            streamerId = fallbackEntry[0];
            targetBroadcaster = fallbackEntry[1];
            autoAssigned = true;
            console.log(`🔀 觀眾 ${viewerId} 無指定主播，自動分配至 ${streamerId}`);
        }
    }
    ...
    // 將基礎資訊附加到 WebSocket，後續聊天/診斷可直接取得
    wss.viewerId = viewerId;
    wss.streamerId = streamerId;
    wss.broadcasterId = streamerId;
    ...
    // 發送確認訊息，包含分配的用戶信息和主播信息
        ...
        console.log('目標主播不存在或不可用:', streamerId, '原始請求:', requestedStreamerId || 'default');
        // 發送主播列表給觀眾，讓他們選擇
...
function handleStreamStart(message) {
    ...
    // 獲取當前在線觀眾列表
    const onlineViewers = Array.from(broadcaster.viewers.keys());
    console.log(`🎯 主播開始直播時有 ${onlineViewers.length} 個在線觀眾`);
    
    // 立即向主播發送在線觀眾列表，讓主播建立 WebRTC 連接
    if (broadcaster.ws && broadcaster.ws.readyState === WebSocket.OPEN) {
        broadcaster.ws.send(JSON.stringify({
            ...
            message: onlineViewers.length > 0 ? 
                `已為 ${onlineViewers.length} 個在線觀眾建立連接` : 
                '目前沒有觀眾在線'
        }));
        console.log(`✅ 已向主播 ${broadcasterId} 發送在線觀眾列表:`, onlineViewers);
    }
...
function handleOffer(message) {
    ...
    if (!broadcaster) {
        console.log('❌ 找不到主播:', message.broadcasterId, '現有主播:', Array.from(activeBroadcasters.keys()));
        return;
    }
    ...
    if (message.viewerId && broadcaster.viewers.has(message.viewerId)) {
        ...
        // 嘗試在所有主播中查找該觀眾 (Fallback)
        let found = false;
        for (const [bId, bData] of activeBroadcasters) {
            if (bData.viewers.has(message.viewerId)) {
                console.log(`⚠️ 觀眾 ${message.viewerId} 實際上在主播 ${bId} 的列表中`);
                found = true;
            }
        }
        if (!found) console.log('❌ 觀眾不在任何主播的列表中');
    }
}
...
function handleChatMessage(ws, message) {
    ...
    if (!broadcasterId) {
        if (ws.viewerId) {
            const viewerData = allViewers.get(ws.viewerId);
            if (viewerData && viewerData.streamerId) {
                broadcasterId = viewerData.streamerId;
            }
        }
    }
    ...
    if (!broadcasterId) {
        for (const connection of activeConnections.values()) {
            if (connection.ws === ws) {
                if (connection.type === 'broadcaster' && connection.userId) {
                    broadcasterId = connection.userId;
                } else if (connection.type === 'viewer' && connection.streamerId) {
                    broadcasterId = connection.streamerId;
                }
            ...
            if (broadcaster) {
                ...
                if (broadcaster.ws && broadcaster.ws.readyState === WebSocket.OPEN) {
                    try {
                        ...
                        console.log('✅ 聊天訊息已發送給主播:', broadcasterId, '內容:', messageText.substring(0, 20));
                    } catch (error) {
                        console.error('❌ 發送訊息給主播失敗:', error);
                    }
                }
    ...
    try {
        ...
        if (user && user.id) {
            try {
                ...
                console.error('⚠️ 產生重設密碼連結失敗:', tokenError);
            }
        }

        res.json({
            success: true,
            message: '如果該電子郵件已註冊，我們將發送重設密碼連結請查收'
        });
    } catch (error) {
        console.error('忘記密碼流程失敗:', error);
        res.status(500).json({
            success: false,
            message: '伺服器錯誤，請稍後再試'
        });
...
app.post('/api/auth/reset-password', async (req, res) => {
    ...
    try {
        ...
        await db.invalidatePasswordResetTokens(tokenRecord.user_id);
...
// 儲值系統 API
app.post('/api/topup/lookup', async (req, res) => {
    const accountRaw = (req.body.account || '').trim();

    if (!accountRaw) {
        return res.status(400).json({
            success: false,
            message: '請提供使用者帳號或電子郵件'
        });
    }

    let user;
    const normalized = accountRaw.toLowerCase();
    const isEmail = normalized.includes('@');

    try {
        user = isEmail ? await db.getUserByEmail(normalized) : await db.getUserByUsername(normalized);
    } catch (error) {
    ...
    try {
        ...
        const history = await db.getTopUpHistory(user.id, 10);

        res.json({
            success: true,
            user,
            balance,
            history: history.map(mapTopUpRow)
        });
    } catch (error) {
        console.error('查詢儲值紀錄失敗:', error);
        res.status(500).json({
            success: false,
            message: '無法取得儲值紀錄'
        });
...
app.post('/api/topup', async (req, res) => {
    ...
    const autoConfirm = Boolean(req.body.autoConfirm);
    ...
    try {
        await db.getUserById(userId); // 確認用戶存在

        const record = await db.createTopUpRecord({
            userId,
            operatorId: req.session && req.session.userId ? req.session.userId : null,
            amount,
            paymentMethod,
            status: 'pending',
            reference,
        ...
        let topUpRow = await db.getTopUpById(record.id);
        let balance = await db.getUserBalance(userId);

        if (autoConfirm) {
            try {
                const confirmed = await db.confirmTopUp(record.id);
                topUpRow = confirmed.topUp;
                balance = confirmed.balance;
            } catch (confirmError) {
                console.warn('自動確認儲值失敗:', confirmError);
            }
        }

        const history = await db.getTopUpHistory(userId, 10);

        res.json({
            success: true,
            topUp: mapTopUpRow(topUpRow),
            balance,
            history: history.map(mapTopUpRow)
        });
    } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: '建立儲值紀錄失敗，請稍後再試'
        });
    }
});

app.post('/api/topup/confirm', async (req, res) => {
    const topUpId = parseInt(req.body.topUpId, 10);
    ...
    try {
        const result = await db.confirmTopUp(topUpId);
        const history = await db.getTopUpHistory(result.topUp.user_id, 10);

        res.json({
            success: true,
            topUp: mapTopUpRow(result.topUp),
            balance: result.balance,
            history: history.map(mapTopUpRow)
        });
    } catch (error) {
        if (error && error.message === '儲值紀錄不存在') {
            return res.status(404).json({
                success: false,
                message: '找不到儲值紀錄'
            });
        ...
        res.status(500).json({
            success: false,
            message: '確認儲值失敗，請稍後再試'
        });
    }
});

app.post('/api/topup/cancel', async (req, res) => {
    const topUpId = parseInt(req.body.topUpId, 10);
    const reasonRaw = req.body.reason ? String(req.body.reason) : null;
    ...
    try {
        const result = await db.cancelTopUp(topUpId, reason);
        const history = await db.getTopUpHistory(result.topUp.user_id, 10);
        ...
        if (error && error.message.includes('無法取消')) {
            return res.status(400).json({
                success: false,
                message: error.message
            });
        ...
        console.error('取消儲值失敗:', error);
        res.status(500).json({
            success: false,
            message: '取消儲值失敗，請稍後再試'
        });
    }
});
...
app.get('/api/topup/history', async (req, res) => {
    ...
    const accountRaw = (req.query.account || '').trim();
    ...
    try {
        const history = await db.getTopUpHistory(user.id, limit);
        const balance = await db.getUserBalance(user.id);
...
app.get('/api/wallet/summary', requireAuth, async (req, res) => {
    try {
        ...
        const topUps = await db.getTopUpHistory(userId, 5);
        const gifts = await db.getGiftHistory(userId, 5);

        res.json({
            success: true,
            balance,
            topUps: topUps.map(mapTopUpRow),
            gifts: gifts.map(mapGiftRow)
        });
    } catch (error) {
        console.error('取得錢包摘要失敗:', error);
        res.status(500).json({
            success: false,
            message: '無法取得錢包資訊'
        });
...
app.post('/api/gifts/send', requireAuth, async (req, res) => {
    const giftType = (req.body.giftType || '').toLowerCase();
    const requestedBroadcasterId = typeof req.body.broadcasterId !== 'undefined' && req.body.broadcasterId !== null
        ? String(req.body.broadcasterId).trim()
        : '';
    const broadcasterId = requestedBroadcasterId || null;
    const activeBroadcasterEntry = broadcasterId ? activeBroadcasters.get(broadcasterId) : null;
    const resolvedBroadcasterUserId = await resolveBroadcasterUserId({
        broadcasterId,
        activeEntry: activeBroadcasterEntry
    });
    ...
    try {
        const senderUser = await db.getUserById(req.session.userId).catch(() => null);
        const giftResult = await db.spendBalanceForGift({
            userId: req.session.userId,
            broadcasterId,
            giftType,
            amount: GIFT_CATALOG[giftType].amount,
            broadcasterUserId: resolvedBroadcasterUserId
        });

        const giftPayload = {
            type: giftType,
            label: GIFT_CATALOG[giftType].label,
            amount: GIFT_CATALOG[giftType].amount
        };
        ...
        res.json({
            ...
            gift: giftPayload,
            broadcasterBalance: giftResult.broadcasterBalance ?? null
        });

        const broadcasterEntry = activeBroadcasterEntry;
        if (broadcasterEntry && broadcasterEntry.ws && broadcasterEntry.ws.readyState === WebSocket.OPEN) {
            const giftEvent = {
                type: 'gift_received',
                broadcasterId,
                transactionId: giftResult.transactionId,
                gift: giftPayload,
                giftType,
                giftLabel: giftPayload.label,
                amount: giftPayload.amount,
                broadcasterBalance: giftResult.broadcasterBalance ?? null,
                broadcasterUserId: giftResult.broadcasterUserId ?? null,
                from: senderUser
                    ? {
                        id: senderUser.id,
                ...
                timestamp: Date.now(),
                message: senderUser
                    ? `${senderUser.displayName || senderUser.username} 送出 ${giftPayload.label}`
                    : undefined
            };

            sendJSON(broadcasterEntry.ws, giftEvent);
        } else if (!giftResult.broadcasterUserId) {
            console.warn('⚠️ 找不到對應的主播帳號可入帳:', broadcasterId);
        }
    } catch (error) {
        if (error && error.message === '餘額不足') {
            return res.status(400).json({
                success: false,
                message: '餘額不足，請先儲值'
            });
