...
function initBroadcasterWalletElements() {
    if (broadcasterWalletState.initialized) {
        return;
    }

    broadcasterWalletState.container = document.getElementById('broadcasterWalletDisplay');
    broadcasterWalletState.amount = document.getElementById('broadcasterWalletAmount');

    if (broadcasterWalletState.container && broadcasterWalletState.amount) {
        broadcasterWalletState.initialized = true;
    }
}

function teardownBroadcasterWalletTimer() {
    if (broadcasterWalletState.refreshTimer) {
        clearInterval(broadcasterWalletState.refreshTimer);
        broadcasterWalletState.refreshTimer = null;
    }
}
...
function updateBroadcasterWalletAmount(value) {
    initBroadcasterWalletElements();
    if (!broadcasterWalletState.initialized) {
        return;
    }

    if (typeof value === 'number' && Number.isFinite(value)) {
        broadcasterWalletState.amount.textContent = formatCurrency(value);
        broadcasterWalletState.container.classList.remove('wallet-hidden');
    } else if (typeof value === 'string') {
        broadcasterWalletState.amount.textContent = value;
        broadcasterWalletState.container.classList.add('wallet-hidden');
    } else {
        broadcasterWalletState.amount.textContent = 'NT$ --';
        broadcasterWalletState.container.classList.add('wallet-hidden');
    }
}

async function refreshBroadcasterWalletBalance(options = {}) {
    if (!currentUser || currentUser.isGuest) {
        updateBroadcasterWalletAmount('NT$ --');
        return;
    }

    initBroadcasterWalletElements();
    if (!broadcasterWalletState.initialized || broadcasterWalletState.loading) {
        return;
    }

    const now = Date.now();
    if (!options.force && broadcasterWalletState.lastUpdated && now - broadcasterWalletState.lastUpdated < 4000) {
        return;
    }

    broadcasterWalletState.loading = true;
    try {
        const response = await fetch('/api/wallet/summary');
        const data = await response.json();

        if (response.ok && data.success) {
            updateBroadcasterWalletAmount(data.balance);
            currentUser.balance = data.balance;
            broadcasterWalletState.lastUpdated = now;
        } else if (options.clearOnError) {
            updateBroadcasterWalletAmount('NT$ --');
        }
    } catch (error) {
        console.error('更新主播錢包餘額失敗:', error);
        if (options.clearOnError) {
            updateBroadcasterWalletAmount('NT$ --');
        }
    } finally {
        broadcasterWalletState.loading = false;
    }
}

function setupBroadcasterWalletUI() {
    initBroadcasterWalletElements();
    teardownBroadcasterWalletTimer();
    ...
    refreshBroadcasterWalletBalance({ force: true });
    broadcasterWalletState.refreshTimer = setInterval(() => {
        refreshBroadcasterWalletBalance();
    }, 30000);
}

function handleStreamerGiftEvent(payload = {}) {
    console.log('🎁 [GIFT] 收到禮物事件:', payload);

    if (typeof payload.broadcasterBalance === 'number') {
        updateBroadcasterWalletAmount(payload.broadcasterBalance);
        broadcasterWalletState.lastUpdated = Date.now();
    } else {
        refreshBroadcasterWalletBalance();
    }

    if (typeof addMessage === 'function') {
        const senderName = (payload.from && (payload.from.displayName || payload.from.username)) || '觀眾';
        const giftInfo = payload.gift || {};
        const amountValue = typeof giftInfo.amount === 'number' ? giftInfo.amount : payload.amount;
        const amountLabel = typeof amountValue === 'number' ? `（${formatCurrency(amountValue)}）` : '';
        const giftLabel = giftInfo.label || payload.giftLabel || payload.giftType || '禮物';
        addMessage('系統', `🎁 ${senderName} 送出 ${giftLabel}${amountLabel}`);
    }

    if (payload.message && typeof displaySystemMessage === 'function') {
        displaySystemMessage(payload.message);
    }

    // 顯示禮物特效
    if (typeof showGiftAnimation === 'function') {
...
const GIFT_ICONS = {
    ...
    hug: 'fa-hand-holding-heart',
    rose: 'fa-rose',
    icecream: 'fa-ice-cream',
    balloon: 'fa-balloon',
    fireworks: 'fa-fireworks',
    magicwand: 'fa-wand-magic-sparkles',
    trophy: 'fa-trophy',
...
// 顯示禮物動畫
function showGiftAnimation(payload) {
    const giftType = payload.giftType || (payload.gift ? payload.gift.type : 'heart');
    const iconClass = GIFT_ICONS[giftType] || 'fa-gift';
    const color = getGiftColor(giftType);
    
    const container = document.querySelector('.stream-area') || document.body;
    const element = document.createElement('div');
    element.className = 'gift-animation';
    element.innerHTML = `<i class="fas ${iconClass}"></i>`;
    
    // 隨機位置
    const randomLeft = 20 + Math.random() * 60; // 20% - 80%
...
window.handleStreamerGiftEvent = handleStreamerGiftEvent;
...
async function logout() {
    try {
        ...
        try {
            ...
            console.warn('⚠️ 登出回應解析失敗，將依預設流程處理', parseError);
        }
        
        if (data.success) {
            console.log('✅ 伺服器已確認登出');
        } else {
            const message = data.message || '未知原因';
            console.warn('⚠️ 登出回應非成功狀態:', message);
            showErrorModal('登出失敗：' + message);
        }
    } catch (error) {
        ...
        try {
            ...
            console.warn('⚠️ 清除客戶端登入狀態失敗:', storageError);
        }
