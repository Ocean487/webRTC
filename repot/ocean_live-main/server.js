    ...
    ws.on('message', async function message(data) {
        ...
        try {
            ...
            case 'stream_start':
                ...
                broadcastToViewers({
                    type: 'stream_start',
                    title: currentStreamTitle || '直播中',
                    message: '直播即將開始'
                });

                // Acknowledge the broadcaster
                sendJSON(ws, { type: 'ack', event: 'stream_start', ok: true });

                // If the broadcaster requested the list of current viewers, send it.
                if (message.requestViewers) {
                    const viewerList = Array.from(viewers.keys());
                    console.log(`發送觀眾列表給主播: ${viewerList.length} 人`);
                    sendJSON(ws, {
                        type: 'online_viewers',
                        viewers: viewerList,
...
function handleChatDisconnect(ws) {
    ...
    if (chatRooms.has(broadcasterId)) {
        ...
        // 如果聊天室沒有任何用戶，考慮清理（保留聊天記錄）
        if (!chatRoom.broadcaster && chatRoom.viewers.size === 0) {
            console.log(`聊天室 ${broadcasterId} 已空，保留聊天記錄`);
            // 可以選擇清理聊天室或保留用於重新連接
            // chatRooms.delete(broadcasterId);
        }
    }
