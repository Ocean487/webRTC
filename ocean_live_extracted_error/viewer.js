...
function getStreamerIdFromUrl() {
    ...
    let streamerId = urlParams.get('streamer') || 'default';
    
    // 確保 streamerId 格式與 activeBroadcasters 的鍵值格式一致
    if (streamerId !== 'default' && !streamerId.startsWith('broadcaster_')) {
        streamerId = `broadcaster_${streamerId}`;
    }
    
    console.log('🔍 [VIEWER] 從 URL 獲取的主播ID:', streamerId);
    return streamerId;
}
...
function initializeChatSystem() {
    ...
    console.log('🔍 [VIEWER] 觀眾頁面 broadcasterId:', targetStreamerId);
    ...
    window.chatSystem = new ChatSystem({
        ...
        broadcasterId: targetStreamerId, // 使用已格式化的主播ID
        selectors: {
    ...
    console.log('[VIEWER] 聊天系統初始化完成，用戶名:', username, '主播ID:', streamerId);
}
