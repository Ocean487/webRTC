...
function checkAudioOutputSupport() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
        window.addMessage('系統', '⚠️ 您的瀏覽器不支援裝置列舉功能');
...
async function loadDevices() {
    try {
        ...
        window.addMessage('系統', '⚠️ 無法檢測音視訊裝置，請檢查瀏覽器權限');
    }
}
...
async function startStream() {
    try {
        ...
        if (currentAudioOutput && currentAudioOutput !== 'default') {
            try {
                if (localVideo.setSinkId) {
                    ...
                    window.addMessage('系統', `🔊 音訊輸出端已設置為: ${deviceName}`);
                    ...
                    try {
                        ...
                        window.addMessage('系統', '⚠️ 請點擊視訊畫面以開始播放音訊');
                    }
                }
            } catch (error) {
                console.warn('設置音訊輸出端失敗:', error);
                window.addMessage('系統', '⚠️ 音訊輸出端設置失敗，使用預設輸出端');
            }
        } else {
            window.addMessage('系統', '🔊 使用預設音訊輸出端');
        ...
        window.addMessage('系統', '🎉 直播已開始！觀眾可以看到你的畫面了');
        ...
        setTimeout(() => {
            if (streamingSocket && streamingSocket.readyState === WebSocket.OPEN) {
                ...
                window.addMessage('系統', `🔄 直播已開始，標題: ${finalTitle}`);
                window.addMessage('系統', '🔄 正在為現有觀眾建立連接...');
            }
        }, 1000);
        ...
        window.addMessage('系統', '❌ ' + errorMessage);
...
function stopStream() {
    ...
    window.addMessage('系統', '📺 直播已結束，感謝觀看！');
}
...
async function toggleVideo() {
    ...
    window.addMessage('系統', isVideoEnabled ? '📹 視訊已開啟' : '📹 視訊已關閉');
...
async function toggleAudio() {
    ...
    window.addMessage('系統', isAudioEnabled ? '🎤 音訊已開啟' : '🎤 音訊已關閉');
...
async function switchCamera() {
    ...
    try {
        ...
        window.addMessage('系統', `🔄 已切換到${cameraType}，音訊保持不變`);
        
        // 更新所有 WebRTC 連接的軌道
        await updateAllPeerConnections();
    } catch (error) {
        console.error('切換鏡頭失敗:', error);
        window.addMessage('系統', '❌ 鏡頭切換失敗');
    }
}
...
async function shareScreen() {
    try {
        ...
        window.addMessage('系統', '🖥️ 螢幕分享已開始，音訊保持不變');

        // 監聽螢幕分享結束
        screenStream.getVideoTracks()[0].onended = () => {
            window.addMessage('系統', '🖥️ 螢幕分享已結束');
            // 可以選擇切回攝影機或結束直播
        };
...
async function changeQuality() {
    ...
    try {
        ...
        if (videoTracks.length > 0) {
            ...
            document.getElementById('currentQuality').textContent = currentQuality + 'p';
            window.addMessage('系統', `📺 畫質已切換為 ${currentQuality}p，音訊保持不變`);
        }

    } catch (error) {
        console.error('畫質切換失敗:', error);
        window.addMessage('系統', '❌ 畫質切換失敗');
    }
}
...
async function switchVideoDevice() {
    ...
    try {
        ...
        window.addMessage('系統', '📹 攝影機已切換，音訊保持不變');
...
async function checkAudioOutputStatus() {
    try {
        ...
        if (!localVideo.setSinkId) {
            window.addMessage('系統', '⚠️ 瀏覽器不支援音訊輸出端切換');
            return;
        }
        ...
        window.addMessage('系統', statusMessage);
        ...
        window.addMessage('系統', '❌ 檢查音訊輸出端狀態失敗');
    }
}
    ...
    try {
        ...
        if (isStreaming && localStream) {
            try {
                ...
                window.addMessage('系統', `🔊 正在測試音訊輸出端: ${deviceName}`);
                ...
                setTimeout(async () => {
                    ...
                    window.addMessage('系統', '🔊 音訊測試完成，已恢復原輸出端');
                }, 3000);
                
            } catch (error) {
                console.error('音訊測試失敗:', error);
                window.addMessage('系統', '❌ 音訊測試失敗: ' + error.message);
            }
        } else {
            // 如果沒有直播，創建測試音訊
            ...
            setTimeout(() => {
                oscillator.stop();
                window.addMessage('系統', '🔊 音訊測試完成');
            }, 1000);
        }
        
    } catch (error) {
        console.error('音訊測試失敗:', error);
        window.addMessage('系統', '❌ 音訊測試失敗');
    }
}
...
async function switchAudioOutput() {
    try {
        ...
        if (!isStreaming) {
            ...
            window.addMessage('系統', '🔊 音訊輸出端已設定，開始直播後生效');
            return;
        }
        ...
        window.addMessage('系統', `🔊 音訊輸出端已切換至: ${deviceName}`);
        ...
        if (localVideo.paused) {
            try {
                await localVideo.play();
                console.log('音訊已開始播放');
                window.addMessage('系統', '▶️ 音訊已開始播放');
            } catch (error) {
...
function simulateViewers() {
    ...
    if (Math.random() < 0.3) {
        ...
        window.addMessage(randomUsername, randomMessage);
    }
...
function getUserAvatar(username) {
    ...
    } else if (username === '主播' || username === window.getCurrentUserDisplayName()) {
        // 使用當前用戶的頭像
        return window.getCurrentUserAvatarHtml();
    } else {
        // 普通用戶顯示用戶名的第一個字母
...
function getCurrentUser() {
    ...
    } else if (window.getCurrentUserDisplayName) {
...
function sendMessage() {
    ...
    if (streamingSocket && streamingSocket.readyState === WebSocket.OPEN) {
        ...
        window.addMessage('系統', '⚠️ 網路連接異常，訊息無法發送', 'system');
    }
...
async function requestPermissions() {
    try {
        await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
        window.addMessage('系統', '✅ 權限已獲得，請重新點擊開始直播');
        ...
        window.addMessage('系統', '❌ 權限請求失敗，請檢查瀏覽器設定');
    }
}
...
function simulateInitialActivity() {
    // 模擬一些初始的聊天訊息
    setTimeout(() => {
        window.addMessage('系統', '👋 歡迎來到直播平台！');
    }, 1000);
    
    setTimeout(() => {
        window.addMessage('系統', '💡 提示：點擊開始直播來啟動您的攝影機');
    }, 3000);
}
...
function connectToStreamingServer() {
    ...
    try {
        ...
        streamingSocket.onopen = function() {
            ...
            window.addMessage('系統', '🔗 已連接到直播服務器');
        ...
        streamingSocket.onclose = function(event) {
            ...
            window.addMessage('系統', `⚠️ 與直播服務器斷開連接 (Code: ${event.code})`);
        ...
        setTimeout(() => {
            if (streamingSocket && streamingSocket.readyState === WebSocket.CONNECTING) {
                ...
                window.addMessage('系統', '⚠️ 連接超時，請檢查網路或伺服器狀態');
            }
        }, 10000); // 10秒超時
        
    } catch (error) {
        console.error('❌ 無法創建 WebSocket 連接:', error);
        window.addMessage('系統', '❌ 無法連接到直播服務器: ' + error.message);
    }
}
...
function handleServerMessage(data) {
    ...
    window.addMessage('系統', `📨 收到消息: ${data.type}`);
    
    switch (data.type) {
        case 'broadcaster_joined':
            window.addMessage('系統', '✅ 主播已成功加入直播間');
            break;
        ...
        case 'online_viewers':
            ...
            window.addMessage('系統', `🎯 收到在線觀眾列表: ${data.viewers ? data.viewers.length : 0} 個`);
...
function handleOnlineViewers(data) {
    ...
    console.log(`收到在線觀眾列表，共 ${viewers.length} 人。`);

    if (viewers.length === 0) {
        window.addMessage('系統', '📺 目前沒有觀眾，等待觀眾加入...');
        return;
    }
    ...
    const tryConnect = (retryCount = 0) => {
        if (!isStreaming || !localStream) {
            if (retryCount < 5) {
                console.warn(`直播狀態尚未就緒，${500 * (retryCount + 1)}ms 後重試...`);
                setTimeout(() => tryConnect(retryCount + 1), 500 * (retryCount + 1));
            } else {
                console.error('❌ 連接失敗：直播狀態長時間未就緒。');
                window.addMessage('系統', '❌ 無法為觀眾建立連接，請嘗試重新開始直播。');
            }
            return;
        }
            ...
            if (!peerConnections.has(viewerId)) {
                ...
                setTimeout(() => {
                    try {
                        ...
                        window.addMessage('系統', `❌ 為觀眾 ${viewerId.substr(-3)} 建立連接失敗`);
                    }
                }, index * 200); // 縮短延遲，加快連接速度
            }
        });
    };
...
function handleViewerJoin(data) {
    ...
    if (isStreaming && localStream) {
        ...
        if (!peerConnections.has(data.viewerId)) {
            ...
            window.addMessage('系統', `🔄 正在為觀眾 ${data.viewerId.substr(-3)} 建立連接...`);
            ...
            window.addMessage('系統', `ℹ️ 觀眾 ${data.viewerId.substr(-3)} 已有連接`);
        }
    } else {
        if (!isStreaming) {
            console.log('⚠️ 主播尚未開始直播');
            window.addMessage('系統', `⚠️ 觀眾 ${data.viewerId.substr(-3)} 加入，但直播尚未開始`);
        }
        if (!localStream) {
            console.log('⚠️ 本地媒體流未建立');
            window.addMessage('系統', `⚠️ 無法為觀眾建立連接：攝影機未啟用`);
        }
    }
}
...
function handleViewersNeedConnection(data) {
    ...
    console.log('當前直播狀態 isStreaming:', isStreaming);
    console.log('本地媒體流 localStream:', localStream ? '已建立' : '未建立');
    window.addMessage('系統', data.message);
    
    if (!isStreaming) {
        console.log('⚠️ 直播尚未開始，無法建立觀眾連接');
        window.addMessage('系統', '⚠️ 直播尚未開始，無法建立觀眾連接');
        return;
    }
    
    if (!localStream) {
        console.log('⚠️ 本地媒體流未建立，無法建立觀眾連接');
        window.addMessage('系統', '⚠️ 攝影機未啟用，無法建立觀眾連接');
        return;
    }
    ...
    data.viewers.forEach(viewerId => {
        if (!peerConnections.has(viewerId)) {
            ...
            window.addMessage('系統', `🔄 正在為等待的觀眾 ${viewerId.substr(-3)} 建立連接...`);
            createPeerConnection(viewerId);
            sendStreamToViewer(viewerId);
        } else {
...
function createPeerConnection(viewerId) {
    try {
        ...
        if (localStream) {
            ...
            window.addMessage('系統', `❌ 無法為觀眾 ${viewerId.substr(-3)} 建立連接：本地串流不存在`);
            return;
        }
        ...
        peerConnection.onconnectionstatechange = function() {
            ...
            switch (state) {
                case 'connecting':
                    window.addMessage('系統', `🔄 正在與觀眾 ${viewerId.substr(-3)} 建立連接...`);
                    break;
                case 'connected':
                    console.log(`✅ 觀眾 ${viewerId.substr(-3)} 視訊連接成功`);
                    window.addMessage('系統', `✅ 觀眾 ${viewerId.substr(-3)} 連接成功！`);
                    break;
                case 'disconnected':
                    console.log(`⚠️ 觀眾 ${viewerId.substr(-3)} 連接中斷`);
                    window.addMessage('系統', `⚠️ 觀眾 ${viewerId.substr(-3)} 連接中斷`);
                    break;
                case 'failed':
                    console.log(`❌ 觀眾 ${viewerId.substr(-3)} 連接失敗`);
                    window.addMessage('系統', `❌ 觀眾 ${viewerId.substr(-3)} 連接失敗，將嘗試重連...`);
                ...
                case 'closed':
                    ...
                    window.addMessage('系統', `🔒 觀眾 ${viewerId.substr(-3)} 已離開`);
                    peerConnections.delete(viewerId);
                    break;
            }
        };
...
async function sendStreamToViewer(viewerId) {
    ...
    if (!peerConnection) {
        ...
        window.addMessage('系統', `❌ 找不到觀眾 ${viewerId.substr(-3)} 的連接`);
        return;
    }
    
    try {
        console.log('為觀眾', viewerId, '創建 WebRTC offer');
        window.addMessage('系統', `🔄 正在為觀眾 ${viewerId.substr(-3)} 創建連接...`);
        ...
        if (streamingSocket && streamingSocket.readyState === WebSocket.OPEN) {
            ...
            window.addMessage('系統', `📤 已向觀眾 ${viewerId.substr(-3)} 發送連接請求`);
        } else {
            console.error('WebSocket 連接不可用，無法發送 offer');
            window.addMessage('系統', `❌ 無法發送連接請求：WebSocket 未連接`);
        }
        
    } catch (error) {
        console.error('發送串流失敗:', error);
        window.addMessage('系統', `❌ 發送串流給觀眾 ${viewerId.substr(-3)} 失敗: ${error.message}`);
    }
}
...
async function handleAnswer(data) {
    ...
    if (peerConnection) {
        ...
        window.addMessage('系統', `❌ 找不到觀眾 ${data.viewerId.substr(-3)} 的連接`);
    }
}
...
function handleChatMessage(data) {
    ...
    if (data.viewerId && data.viewerId !== 'system') { 
        ...
        window.addMessage(viewerName, messageText);
        ...
        // 系統消息
        window.addMessage('系統', messageText);
        console.log('收到系統訊息:', messageText);
    } else {
        // 其他消息
        window.addMessage(userName, messageText);
...
async function updateAllPeerConnections() {
    ...
    try {
        window.addMessage('系統', '🔄 正在更新所有觀眾的視訊軌道...');
        ...
        window.addMessage('系統', '✅ 所有觀眾的視訊軌道已更新');
    } catch (error) {
        console.error('更新 WebRTC 連接失敗:', error);
        window.addMessage('系統', '❌ 更新視訊軌道失敗');
    }
}
